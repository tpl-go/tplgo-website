...
const base='http://127.0.0.1:3157',out='.tmp/uf1/admin-browser';await mkdir(out,{recursive:true});
...
try{for(const [width,height] of [[1365,1000],[768,1024],[390,844]]){
 ...
 published=true;await page.evaluate(()=>window.dispatchEvent(new Event('focus')));await overview.getByRole('button',{name:'Published Later Domain',exact:true}).waitFor();
 ...
 fail=true;await page.evaluate(()=>window.dispatchEvent(new Event('focus')));await page.getByText('Updates are unavailable. Previously loaded results may be out of date.',{exact:true}).first().waitFor();fail=false;await page.getByRole('button',{name:'Retry',exact:true}).first().click();await overview.getByRole('button',{name:'eSIM Provider',exact:true}).waitFor();
