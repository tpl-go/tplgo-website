...
const JSON_PATH = path.join(OUT_DIR, "phase4b-flight-round-trip-browser-result.json");
const MD_PATH = path.join(OUT_DIR, "phase4b-flight-round-trip-browser-report.md");
...
async function main() {
  ...
  try {
    ...
    result.status =
      ...
      result.uniqueBookingRef?.ok &&
      result.backendStart?.httpStatus === 201 &&
      result.backendConfirm?.httpStatus === 200 &&
      result.backendRefs?.ok &&
      result.walletGuard?.ok &&
      result.confirmation?.ok &&
      result.myBooking?.ok &&
      result.viewDetail?.ok &&
      result.manageReadPath?.ok &&
      result.fallback?.ok &&
      result.refreshDuplicateGuard?.ok &&
      result.dataIntegrity?.ok
        ? "passed"
        : "failed";
  } finally {
    await browser.close();
  ...
  console.log(`Phase 4B Flight Round Trip browser certification: ${result.status}`);
...
async function runFallbackJourney(browser, auth) {
  ...
  try {
    const reviewData = buildReviewData({
      onwardFlightNumber: "PA-499",
      returnFlightNumber: "PA-594",
    });
...
function verifyDataIntegrity({ reviewData, confirmation, payload, saved }) {
  const onwardSegment = reviewData?.journeys?.[0]?.segments?.[0] || {};
  const returnSegment = reviewData?.journeys?.[1]?.segments?.[0] || {};
  const savedOnwardSegment = payload?.reviewData?.journeys?.[0]?.segments?.[0] || {};
  const savedReturnSegment = payload?.reviewData?.journeys?.[1]?.segments?.[0] || {};
  const checks = {
    ...
    bookingType: payload?.reviewData?.bookingType === "roundTrip",
    journeyCount: payload?.reviewData?.journeys?.length === 2,
    onwardRoute:
      savedOnwardSegment.fromCode === onwardSegment.fromCode &&
      savedOnwardSegment.toCode === onwardSegment.toCode,
    returnRoute:
      savedReturnSegment.fromCode === returnSegment.fromCode &&
      savedReturnSegment.toCode === returnSegment.toCode,
    onwardAirlineFlight:
      savedOnwardSegment.airline === onwardSegment.airline &&
      savedOnwardSegment.flightNumber === onwardSegment.flightNumber,
    returnAirlineFlight:
      savedReturnSegment.airline === returnSegment.airline &&
      savedReturnSegment.flightNumber === returnSegment.flightNumber,
    onwardTiming:
      savedOnwardSegment.departureTime === onwardSegment.departureTime &&
      savedOnwardSegment.arrivalTime === onwardSegment.arrivalTime,
    returnTiming:
      savedReturnSegment.departureTime === returnSegment.departureTime &&
      savedReturnSegment.arrivalTime === returnSegment.arrivalTime,
    pricing:
...
function buildReviewData(overrides = {}) {
  const onwardFlightNumber = overrides.onwardFlightNumber || "PA-421";
  const returnFlightNumber = overrides.returnFlightNumber || "PA-424";
  return {
    ...
    cabinClass: "Economy",
    pricing: {
      perAdultBaseFare: 8400,
      baseFareTotal: 8400,
      tax: 1180,
      surcharge: 320,
      appliedOffer: 500,
      appliedOfferCode: "FLIGHT250",
      appliedOfferTitle: "Flight Phase 4B Round Trip Offer",
      discount: 0,
      tplCredit: 0,
      totalAmount: 9400,
    },
...
function buildPaymentPayload(reviewData, mobile) {
  return {
    ...
    offerData: {
      code: "FLIGHT250",
      title: "Flight Phase 4B Round Trip Offer",
      description: "Round Trip smoke offer",
      discountAmount: 500,
    },
    walletData: {
      promoUsed: 99999,
...
function renderMarkdown(data) {
  const lines = [
    "# Phase 4B Flight Round Trip Browser Certification Result",
    "",
    `Status: ${data.status}`,
