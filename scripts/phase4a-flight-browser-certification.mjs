...
const JSON_PATH = path.join(OUT_DIR, "phase4a-flight-browser-result.json");
const MD_PATH = path.join(OUT_DIR, "phase4a-flight-browser-report.md");
...
async function main() {
  ...
  console.log(`Phase 4A Flight browser certification: ${result.status}`);
...
async function runPrimaryJourney(browser, auth) {
  ...
  try {
    ...
    await page.waitForURL("**/flights/confirmation", { timeout: 60000 });
    await page.waitForFunction(
...
async function runFallbackJourney(browser, auth) {
  ...
  try {
    const reviewData = buildReviewData({ flightNumber: "PA-499" });
...
function verifyDataIntegrity({ reviewData, confirmation, payload, saved }) {
  const segment = reviewData?.journeys?.[0]?.segments?.[0] || {};
  const checks = {
    ...
    route:
      payload?.reviewData?.journeys?.[0]?.segments?.[0]?.fromCode ===
        segment.fromCode &&
      payload?.reviewData?.journeys?.[0]?.segments?.[0]?.toCode === segment.toCode,
    airlineFlight:
      payload?.reviewData?.journeys?.[0]?.segments?.[0]?.airline ===
        segment.airline &&
      payload?.reviewData?.journeys?.[0]?.segments?.[0]?.flightNumber ===
        segment.flightNumber,
    timing:
      payload?.reviewData?.journeys?.[0]?.segments?.[0]?.departureTime ===
        segment.departureTime &&
      payload?.reviewData?.journeys?.[0]?.segments?.[0]?.arrivalTime ===
        segment.arrivalTime,
    pricing:
...
function buildReviewData(overrides = {}) {
  const flightNumber = overrides.flightNumber || "PA-401";
  return {
    bookingType: "oneWay",
    tripMode: "domestic",
    ...
    cabinClass: "Economy",
    pricing: {
      perAdultBaseFare: 4500,
      baseFareTotal: 4500,
      tax: 620,
      surcharge: 180,
      appliedOffer: 250,
      appliedOfferCode: "FLIGHT250",
      appliedOfferTitle: "Flight Phase 4A Offer",
      discount: 0,
      tplCredit: 0,
      totalAmount: 5050,
    },
...
function renderMarkdown(data) {
  const lines = [
    "# Phase 4A Flight Browser Certification Result",
    "",
    `Status: ${data.status}`,
