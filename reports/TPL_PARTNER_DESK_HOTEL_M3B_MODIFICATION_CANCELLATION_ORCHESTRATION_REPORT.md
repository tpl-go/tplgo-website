...
Authenticated Website confirmation passed: Customer My Booking → Upcoming → Cancel Booking now identifies the existing modification request correctly, keeps modification details out of the cancellation history and explains the conflict guard clearly. No new request or data mutation occurred during this read-only check. **Exact next action:** on native Mobile, open the same booking → Cancel Booking and confirm the notice identifies the open modification request/reference/status and directs the customer to Manage Booking; do not submit. Full corrected cancellation, withdrawal/rejection and export live gates remain pending; HOTEL-M3C is not started.
