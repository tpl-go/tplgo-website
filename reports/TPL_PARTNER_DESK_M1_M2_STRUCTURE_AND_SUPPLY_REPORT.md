...
Customer withdrawal also passed live. Request `TPL-MOD-2AEE29A5` is `WITHDRAWN`/version 2 before Hotel response; booking `TPL-QA-HOTEL-M3B-WD-001` remains confirmed/version 1 with one active allocation and unchanged test payment. Pending jobs are zero, external delivery is zero, and staging/production API health is 200. Future automation direction records instant safe corrections, availability-backed choices, authoritative re-quote, additional-payment and governed TPL Wallet refund hooks as separate later authorities; M3B performs none of those financial/provider actions.

The cancellation stamp is queued for one consistent right-aligned emerald/white treatment across Customer, Partner and Admin. Local verified commits `26c7f9b` and `7cc5ae0` are intentionally held for the next meaningful Website deployment to conserve Vercel credits; current staging deployment remains unchanged. Rejection/no-mutation, timeout/escalation and final cross-surface/export reconciliation remain open. Exact next action is one customer cancellation submission on `TPL-QA-HOTEL-M3B-WD-001`; Hotel/Admin actions remain separate later gates. All preserved program flags remain unchanged and HOTEL-M3C is not started.
...
Actual isolated PostgreSQL passes 10/10; Website focused 9/9 plus lint/diff/244-page build and Mobile focused 19/19 plus type/lint/diff pass. Backend staging release `partner-hotel-m3b-projection-9093ea24d56a9c4530587958b5889d51af81298b` and Website READY `dpl_4KXS1WH5G77hbtRqVYtFTsFWsnGb` are staging-only; the existing Development APK is reused. No migration or data mutation occurred. Payment/refund execution/provider/external delivery remain zero, production remains healthy and untouched. Authenticated read-only customer confirmation plus separate cancellation/withdrawal/rejection and final export reconciliation remain mandatory; HOTEL-M3C is not started.
## HOTEL-M3B continuation — 2026-09-25 — My Booking card action routing

Checkpoint `TPL-PARTNER-HOTEL-M3B-CARD-ACTION-ROUTING-20260925-03` supersedes only the prior View Detail → Open Manage proposal and preserves **HOTEL_M3B_MODIFICATION_CANCELLATION_AUTOMATION_READY_STAGING_PARTIAL_CORRECTED_LIVE_FLOWS_PENDING**. Website `a63d1dc` keeps View Detail read-only, routes the row-level Manage Booking action directly to Modification request and the row-level Cancel Booking action directly to Cancellation request, and removes the mixed request-type selector. Focused 5/5, scoped lint/diff/secret checks and 244-page build pass. READY deployment `dpl_mCjW5V1VcQvC8kmp8Y3QNYurT22s` is staging-only; data, Backend, Mobile, APK, finance/provider delivery and production are unchanged. One authenticated routing recheck remains; do not resubmit.
## HOTEL-M3B continuation — 2026-09-25 — customer Manage Booking placement

Checkpoint `TPL-PARTNER-HOTEL-M3B-CUSTOMER-MANAGE-ROUTING-20260925-02` preserves **HOTEL_M3B_MODIFICATION_CANCELLATION_AUTOMATION_READY_STAGING_PARTIAL_CORRECTED_LIVE_FLOWS_PENDING**. Website `c200567` moves customer request creation/withdrawal to Manage Booking → Change / Cancel Request, keeps View Details read-only, separates background polling from mutation state, and suppresses duplicate submission while an open request exists. Focused 5/5, changed-component lint, diff/secret checks and 244-page build pass. READY deployment `dpl_HfGqrUFx66NAZoYbkPxzUS3CW6Fa` is staging-only; Backend/Mobile/APK/data/finance/provider/production are unchanged. One authenticated read-only recheck remains; the existing request must not be resubmitted.
...
Actual PostgreSQL coverage passes 15/15; affected Backend/Website/Mobile type, build, lint, focused and Hermes checks pass. Backend release `/home/tpladmin/tpl-api-releases/partner-hotel-m3b-drilldown-f8d0e8d99edf4ddb2f437fec59f404ddb67acc35` runs only on staging 4100; READY Website deployment `dpl_5RPWFkmweaXeyyBNDPSXX8ov4ZmD` is assigned only to `staging.tplgo.com`; the existing Development APK/Metro is reused. Fresh backup SHA-256 is `953ccbd20031ac901cbbba32ec1c0d909c3556bd1b62d6fbfc44c1f2e8de6319` with 1,475 restore-list entries.

Canonical readback remains one `CONFIRMED` unacknowledged booking and one preserved `SUBMITTED` modification request, with allocation once and availability 4/8. No finance/provider/external-delivery or production mutation occurred; staging and production API health are 200. Exact next action is the separate authenticated **Acknowledge original booking once** action, followed by readback before sending the request to Admin. Completed statuses and fixed **46 requirements / 213 units**, `PARTNER_PROGRESS_PERCENTAGE_NOT_YET_AUDITABLE`, `MOBILE_USER_PARITY=COMPLETE`, `PARTNER_MOBILE_PARITY=OPEN` and `PHASE_1_STEP_1=OPEN` remain preserved; HOTEL-M3C is not started.
...
## Partner Desk HOTEL-M3B approved modification customer parity — 2026-09-25

Checkpoint `TPL-PARTNER-HOTEL-M3B-MODIFICATION-APPROVED-CUSTOMER-PARITY-20260925-12` preserves **`HOTEL_M3B_MODIFICATION_CANCELLATION_AUTOMATION_READY_STAGING_PARTIAL_CORRECTED_LIVE_FLOWS_PENDING`**. The authenticated customer-origin modification completed Customer → automatic TPL/Hotel routing → Hotel Yes recommendation → TPL Admin approve/apply. Customer status is Approved; Upcoming shows one **Updated booking** marker and View Detail now uses the same canonical revised facts throughout.

Read-only reconciliation for `TPL-MOD-830F5568` shows request `APPROVED`, routing `COMPLETED`, Hotel recommendation `RECOMMEND_APPROVE`, request version 3, booking version 5, three guests and exactly one active allocation. Payment remains `TEST_NO_PAYMENT`. Backend `7b974fcf`/staging release `s8e71-7b974fcf68298ac7710da01d4023df87fbd3c3df` and Website `e5b9204`/READY `dpl_HYqqAmPZLJwToifwpb39a4S8MwSV` are staging-only; Mobile/APK is unchanged. Focused Website 11/11, TypeScript/build and diff checks pass. The isolated PostgreSQL port was unavailable for a fresh delta rerun, so no new PostgreSQL claim is made. Production and finance/provider/external delivery remain untouched.

Exact next action: Partner Website → Command Center → Bookings → `TPL-QA-HOTEL-M3B-MOD-001` → acknowledge the revised booking once, then verify **Ready for check-in** is the next action. Cancellation, withdrawal/rejection, timeout/escalation and final export/live gates remain open; HOTEL-M3C is not started. Fixed **46 requirements / 213 units**, `PARTNER_PROGRESS_PERCENTAGE_NOT_YET_AUDITABLE`, `MOBILE_USER_PARITY=COMPLETE`, `PARTNER_MOBILE_PARITY=OPEN` and `PHASE_1_STEP_1=OPEN` remain preserved.
## Partner Desk HOTEL-M3B Partner modified marker — 2026-09-25

Checkpoint `TPL-PARTNER-HOTEL-M3B-PARTNER-MODIFIED-MARKER-20260925-13` preserves **`HOTEL_M3B_MODIFICATION_CANCELLATION_AUTOMATION_READY_STAGING_PARTIAL_CORRECTED_LIVE_FLOWS_PENDING`**. The operator acknowledged the revised booking once and confirmed the persistent **Modified booking** marker on Partner booking row/detail. Canonical readback shows booking `PARTNER_ACKNOWLEDGED`/v6, allocated, acknowledged, revised dates and three guests retained; request `TPL-MOD-830F5568` remains `APPROVED`/`COMPLETED` with Hotel recommendation `RECOMMEND_APPROVE` and payment `TEST_NO_PAYMENT`.

Website `7490679`, READY staging deployment `dpl_ExNKhb8DXcK5YrWMZV7AdbdX9Kuq`, passes focused 12/12, scoped lint/diff and 245-page Webpack build. Backend/Mobile/APK/data and production remain unchanged. Exact next action is one customer-origin cancellation submission for `TPL-QA-HOTEL-M3B-CAN-001` from Customer My Booking; Hotel/Admin actions remain separate later gates. Full cancellation, withdrawal/rejection, timeout/escalation and export reconciliation remain open; HOTEL-M3C is not started. Fixed **46 requirements / 213 units** and all preserved program flags remain unchanged.
