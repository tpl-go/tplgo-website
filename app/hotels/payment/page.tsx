...
import HotelPaymentTopSummary from "@/app/components/payment/hotel/HotelPaymentTopSummary";
import HotelPaymentTripSecureCard from "@/app/components/payment/hotel/HotelPaymentTripSecureCard";
import HotelPaymentOptionSection from "@/app/components/payment/hotel/HotelPaymentOptionSection";
import HotelPaymentPriceCard from "@/app/components/payment/hotel/HotelPaymentPriceCard";

import type { Hotel, RoomVariant } from "@/app/data/stays/types";

type StoredHotelPaymentPayload = {
  hotel: Hotel;
...
function getHotelCity(hotel: any, searchMeta: any) {
  return (
    hotel?.city ||
    hotel?.cityName ||
    hotel?.locationCity ||
...
function getHotelAddress(hotel: any, searchMeta: any) {
  return (
...
export default function HotelPaymentPage() {
  ...
  const [storedPayload, setStoredPayload] =
    useState<StoredHotelPaymentPayload | null>(null);
  ...
  useEffect(() => {
    const raw =
      typeof window !== "undefined"
        ? sessionStorage.getItem("tplHotelBookingData")
        : null;

    if (!raw) return;

    try {
      const parsed: StoredHotelPaymentPayload = JSON.parse(raw);
      ...
      console.error("Failed to parse hotel payment payload:", error);
    }
  }, []);
      ...
      setTimeLeft((prev) => {
        ...
        if (raw) {
          try {
            ...
            console.error("Failed to update hotel timer:", error);
          }
        }
  ...
  if (!storedPayload?.hotel) {
    return (
      <main className="min-h-screen bg-[#eef3f8] text-black flex items-center justify-center">
        <div className="rounded-xl border border-[#d9e2ec] bg-white p-6 text-[16px] font-semibold text-[#374151]">
          Hotel payment data not found.
        </div>
      </main>
    );
  }

  const hotel = storedPayload.hotel;
  const selectedVariant = storedPayload.selectedVariant;
  const searchMeta = storedPayload.searchMeta;

  const roomPrice =
    storedPayload.fareBreakup?.roomPrice ||
  ...
  const subtotal =
    storedPayload.fareBreakup?.subtotal || roomPrice * rooms * nights;
...
const priceBreakup = {
  ...
  const handleMockPayment = async (shouldSucceed = true) => {
    ...
    if (shouldSucceed) {
      ...
      try {
        ...
        sessionStorage.removeItem("tplHotelBookingData");
      } catch (error) {
        console.error("Failed to store hotel confirmation payload:", error);
      }
  ...
  appliedOffer > 0
    ? {
        ...
        title: storedPayload.appliedOfferTitle || "Offer Applied",
