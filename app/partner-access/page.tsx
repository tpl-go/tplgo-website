...
export default function PartnerAccessPage() {
  ...
  return <main className="mx-auto min-h-[65vh] w-full max-w-2xl px-5 py-12 text-gray-900">
    ...
    {recovery ? <><h1 ref={heading} tabIndex={-1} className="mt-8 text-2xl font-semibold">Recover Existing Partner Access</h1><p className="my-5">Secure recovery is not yet available on staging.</p><div className="flex flex-wrap gap-4"><Link href="/customer-support" className={button}>Support</Link><button onClick={() => setRecovery(false)}>Back</button></div></> : <>
      <h1 ref={heading} tabIndex={-1} className="mt-8 text-2xl font-semibold outline-none">{access?.outcome === "SELECTION_REQUIRED" ? "Choose your Partner profile" : access?.outcome === "NO_LINKED_PROFILE" ? "No Partner profile is linked to this login" : access?.outcome === "SETUP_PENDING" ? "Application approved. Setup pending" : "Partner Access"}</h1>
      {access?.outcome === "SELECTION_REQUIRED" && <PartnerProfileChooser profiles={access.profiles ?? []} onSelected={accept} onSignOut={() => { logout(); window.location.assign("/partner-access"); }} />}
