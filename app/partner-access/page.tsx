...
export default function PartnerAccessPage() {
  const { isAuthLoading, openLoginModal, logout } = useAuth();
  ...
  const refresh = useCallback(() => {
    ...
    void request.finally(() => { if (resolveInFlight.current === request) resolveInFlight.current = null; });
    return request;
  }, [accept]);
  useEffect(() => { if (!isAuthLoading) void refresh(); }, [isAuthLoading, refresh]);
  ...
  return <main className="mx-auto min-h-[65vh] w-full max-w-2xl px-5 py-12 text-gray-900">
    ...
    {recovery ? <><h1 ref={heading} tabIndex={-1} className="mt-8 text-2xl font-semibold">Recover Existing Partner Access</h1><p className="my-5">Secure recovery is not yet available on staging.</p><div className="flex flex-wrap gap-4"><Link href="/customer-support" className={button}>Support</Link><button onClick={() => setRecovery(false)}>Back</button></div></> : <>
      <h1 ref={heading} tabIndex={-1} aria-live="polite" className="mt-8 text-2xl font-semibold outline-none">{isAuthLoading || (loading && !access) ? "Opening your Partner account…" : access?.outcome === "SELECTION_REQUIRED" ? "Choose a business to continue" : access?.outcome === "NO_LINKED_PROFILE" ? "No Partner application found" : access?.outcome === "SETUP_PENDING" ? "Application approved. Setup pending" : "Partner Access"}</h1>
      {access?.outcome === "SELECTION_REQUIRED" && <PartnerProfileChooser profiles={access.profiles ?? []} onSelected={accept} onSignOut={() => { void logout().finally(() => window.location.assign("/partner-access")); }} />}
      {loading && access && <p role="status" className="my-5">Opening your Partner account…</p>}
      {access?.outcome === "NO_LINKED_PROFILE" && <><p className="mt-5">We could not find a Partner application linked to this mobile number, email or Google account.</p><p className="mb-5 mt-2 text-sm text-gray-600">If you previously applied using another mobile number or email, use that login or choose recovery.</p><div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap"><button className={button} disabled={loading} onClick={() => void start()}>Start new application</button><button className="rounded border border-gray-300 px-4 py-3 text-sm" disabled={loading} onClick={() => setRecovery(true)}>Recover existing application</button><button className="rounded border border-gray-300 px-4 py-3 text-sm" disabled={loading} onClick={() => { void logout().finally(() => window.location.assign("/partner-access")); }}>Use another login</button></div></>}
      ...
      {access && ["ACTIVE", "RESTRICTED"].includes(access.outcome) && <><p className="my-5">{access.outcome === "RESTRICTED" ? "Your Partner access requires Support assistance." : "Operational workspace access must be confirmed before continuing. Contact Support."}</p><Link href="/customer-support" className={button}>Support</Link></>}
