...
export default function TrainLiveStatusModal({
  ...
  return createPortal(
    ...
    >
      <div
        ref={panelRef}
        className={`relative w-full max-w-[780px] max-h-[88dvh] overflow-y-auto rounded-[20px] md:rounded-[24px] border bg-white shadow-2xl transition-all duration-200 ${
          flash
      ...
      >
        ...
        <div className="border-b border-slate-200 px-4 md:px-6 py-4 md:py-5">
          ...
          <div className="mt-2 text-sm md:text-[15px] font-medium text-slate-500">
            Last updated: {data.lastUpdated}
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 px-4 md:px-6 py-4 md:py-5">
          ...
          <div className="md:col-span-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
            ...
            <div className="mt-1 text-xl md:text-[22px] font-extrabold text-sky-700">
              {data.statusLabel}
            </div>
          </div>
        </div>
