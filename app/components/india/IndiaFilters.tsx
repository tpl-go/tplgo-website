...
export default function IndiaFilters({
  selectedFilters,
  setSelectedFilters,
  states,
  activeState,
  ...
  const selectedStates = useMemo(() => {
    ...
    const selectedStateFromFilters = states.find((s) =>
      selectedFilters.includes(s)
    );

    return selectedStateFromFilters ? [selectedStateFromFilters] : [];
  }, [selectedFilters, states, activeState]);
  ...
  const toggleState = (state: string) => {
    if (activeState === state) {
      setActiveState("");
      setSelectedFilters(selectedFilters.filter((f) => !states.includes(f)));
      return;
    }

    setActiveState(state);
    setSelectedFilters(
      selectedFilters.filter((f) => !states.includes(f)).concat(state)
    );
  };
            ...
            <div className="mt-4">
              ...
              <div className="space-y-2">
                {visibleStates.map((state) => (
                  <label key={state} className="flex items-center text-black">
                    <input
                      type="checkbox"
              ...
              {filteredStates.length > 4 && (
                ...
                >
                  {showAllStates ? "See Less" : "See All"}
                </button>
              )}
            </div>
          )}
