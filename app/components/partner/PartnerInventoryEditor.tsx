...
export default function PartnerInventoryEditor({data,admin,busy,save,refresh,onDirtyChange}:Props){
 ...
 return <section className={styles.panel} aria-labelledby="partner-inventory-title"><div className={styles.panelHead}><div><h2 id="partner-inventory-title">Inventory & capacity</h2><span className={styles.muted}>Manage the named inventory units and their total capacity.</span></div>{writable&&!editing?<button type="button" onClick={beginCreate}>Add inventory</button>:null}</div>
  ...
  {editing?<form className={styles.editor} onSubmit={event=>{event.preventDefault();void submit();}}><h3>{draft.id?'Edit inventory capacity':'Add inventory'}</h3><label>Service required<select aria-label="Service" required value={draft.serviceScopeId} onChange={event=>change({serviceScopeId:event.target.value})}>{scopes.map(scope=><option key={scope.id} value={scope.id}>{scope.label}</option>)}</select></label><label>Inventory item name required<input aria-label="Inventory item name" required maxLength={120} autoFocus value={draft.label} onChange={event=>change({label:event.target.value})} placeholder="For example, Deluxe Room"/></label><label>Total capacity required<input aria-label="Total capacity" required inputMode="numeric" pattern="[0-9]+" min="0" max="1000000" step="1" value={draft.capacityTotal} onChange={event=>change({capacityTotal:event.target.value})} placeholder="Enter a whole number"/></label>{error?<p role="alert" className={styles.empty}>{error}{error.includes('changed')?<button type="button" onClick={()=>void refresh()}>Refresh records</button>:null}</p>:null}<div className={styles.tools}><button type="submit" disabled={busy}>{busy?'Saving…':draft.id?'Save capacity':'Add inventory'}</button><button type="button" disabled={busy} onClick={cancel}>Cancel</button></div></form>:null}
