...
function Methods({ ownerId }: { ownerId: string }) {
  ...
  return <section aria-label="Verified login methods" className="rounded-2xl border border-slate-200 p-4 text-sm">
    <h2 className="font-semibold text-slate-900">Your verified login methods</h2>
    <p className="mt-2 text-slate-600">Adding a login method does not change your profile or business contacts, or approve Partner or Creator access.</p>
    {methods === null ? <p className="mt-3">{error ? "Login methods unavailable." : "Loading login methods…"}</p> : <ul className="my-3 space-y-2">{methods.map(method => <li key={method.id} className="break-words">{method.label} · {method.provider === "google" ? "Connected" : "Verified"}</li>)}{!methods.some(m => m.provider === "google") && <li>Google is not connected.</li>}</ul>}
    {error && <p role="alert" className="my-3 text-red-700">{error}</p>}
    {message && <p role="status" className="my-3 text-emerald-700">{message}</p>}
    {!flow && methods === null && error && <button type="button" onClick={() => void load()} className="text-blue-700 underline">Retry</button>}
    {!flow && methods && <div className="flex flex-wrap gap-3">{(["mobile", "email"] as const).map(channel => <button key={channel} type="button" disabled={!methods.length} className="rounded-lg border px-3 py-2 disabled:opacity-50" onClick={() => { setMessage(""); setError(""); setMethodId(methods[0]?.id ?? ""); setFlow({ channel, phase: "method" }); }}>Add {channel}</button>)}</div>}
    {flow && <div className="mt-4 space-y-3">
      <p className="font-medium">Add {flow.channel}</p>
      {flow.phase === "method" && <label className="block">First, confirm an existing login method<select aria-label="Existing login method" value={methodId} onChange={e => setMethodId(e.target.value)} className="mt-2 block w-full rounded border p-2">{methods?.map(m => <option key={m.id} value={m.id}>{m.label}</option>)}</select></label>}
      {flow.phase === "google" && <p>Complete Google verification in the separate window using your connected account, then continue here. If the window was blocked, cancel and try again with popups allowed.</p>}
      {flow.phase === "target" && <><label className="block">New {flow.channel}{flow.channel === "mobile" && <CountryDialCodeSelect label="New mobile" value={country} onChange={setCountry} className="my-2 w-full rounded border p-2" />}<input aria-label={`New ${flow.channel}`} type={flow.channel === "email" ? "email" : "tel"} autoComplete="off" value={contact} onChange={e => setContact(e.target.value)} className="mt-2 w-full rounded border p-2" /></label><p className="text-xs text-slate-600">An identifier already used by another account cannot be moved here.</p></>}
      {(flow.phase === "reauth" || flow.phase === "complete") && <label className="block">{flow.phase === "reauth" ? "Code for your existing login method" : "Code for your new login method"}<input aria-label="Verification code" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, ""))} className="mt-2 w-full rounded border p-2" /><span className="mt-1 block text-xs text-slate-600">Codes expire after five minutes. To request another, restart after the waiting period.</span></label>}
      {flow.phase === "reconcile" && <p>The response was interrupted. Retry confirmation within two minutes. This only retrieves the result of this addition.</p>}
      <div className="flex flex-wrap gap-3"><button type="button" onClick={() => void run()} disabled={busy} className="rounded-lg bg-blue-700 px-4 py-2 text-white disabled:opacity-50">{busy ? "Please wait…" : flow.phase === "complete" ? `Confirm Add ${flow.channel}` : flow.phase === "reconcile" ? "Retry confirmation" : "Continue"}</button><button type="button" onClick={cancel} className="rounded-lg border px-4 py-2">Cancel</button></div>
    </div>}
  </section>;
}
