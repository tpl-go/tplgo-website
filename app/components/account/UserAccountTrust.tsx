...
export function UserAccountTrustProvider({ children }: { children: React.ReactNode }) {
  ...
  const profile = savedProfile.status !== "ready" ? { value: savedProfile.status === "loading" ? "Loading personal email…" : "Personal email unavailable", status: "Not confirmed", verified: false } : profileEmail ? { value: profileEmail, status: "Personal email", verified: false }
    : { value: "No personal email added", status: "Not added", verified: false };
...
export function UserSignInDetails() {
  ...
  return <section className="rounded-2xl border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-indigo-50 p-4 shadow-[0_10px_28px_rgba(14,165,233,0.08)]" aria-label="Sign-in details">
    ...
    <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
      {[{ label: "Login email", ...login }, { label: "Personal email", ...profile }].map(item => <article key={item.label} className="min-w-0 rounded-2xl border border-white/80 bg-white p-4 shadow-sm">
