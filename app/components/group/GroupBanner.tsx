...
export default function GroupTourBanner({ slug = "group-tour" }: Props) {
  const imageSlug = slug && slug.length > 0 ? slug : "group-tour";

  const displayName = "Group Tour";

  const images = [
    `/group-tour/banners/${imageSlug}-1.jpg`,
    `/group-tour/banners/${imageSlug}-2.jpg`,
    `/group-tour/banners/${imageSlug}-3.jpg`,
  ];
