...
type ReviewTransportGapAlertProps = {
  gaps: string[];
};

export default function ReviewTransportGapAlert({
  gaps,
}: ReviewTransportGapAlertProps) {
  if (!gaps.length) {
    return (
      <div className="rounded-[1.75rem] border border-emerald-200 bg-emerald-50 p-5 text-emerald-800">
        ...
        <p className="mt-3 text-sm font-semibold leading-6">
          No obvious missing transfer gaps detected from the current review
          payload.
        </p>
      </div>
    );
