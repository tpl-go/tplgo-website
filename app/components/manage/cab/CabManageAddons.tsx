...
import { SectionTitle, formatPrice } from "./CabManageShared";

export type CabVariant = {
  id?: string;
  name?: string;
  cabType?: string;
  vehicleType?: string;
...
type Props = {
  ...
  tripType: string;
  addOnsTotal: number;
  selectedVariant?: CabVariant | null;
  variants?: CabVariant[];
  activeVariantId?: string;
  onVariantChange: (variant: CabVariant) => void;
  quote: CabQuote;
  onContinue: () => void;
};

export default function CabManageAddons({
  cabName,
  tripType,
  addOnsTotal,
  selectedVariant,
    ...
    <div className="space-y-5">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
          {variants.length === 0 ? (
            <div className="rounded-2xl border border-black/5 bg-[#f8f9fb] p-4 text-sm font-semibold text-[#6b7280]">
              Cab variants not available in payload.
            </div>
          ) : (
            variants.map((variant) => {
              ...
              const baseFare = Number(variant.price || 0);
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {variant.seats ? (
                      <Pill label={`${variant.seats} Seats`} />
                    ) : null}

                    {variant.luggage ? (
                      <Pill label={`${variant.luggage} Luggage`} />
                    ) : null}

                    {variant.fuelType ? (
                      <Pill label={variant.fuelType} />
                    ) : null}
                  </div>
      ...
      <div className="rounded-[24px] border border-black/5 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 space-y-3">
          <Row
            label="Current Cab Total"
            value={formatPrice(quote.oldTotal)}
          />

          <Row
            label="New Cab Total"
            value={formatPrice(quote.newTotal)}
          />
          ...
          {quote.settlementMode === "save" && (
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-[#6b7280]">
                  Same fare. Changes can be saved directly.
                </p>
              </div>
...
function getSettlementTitle(mode: CabQuote["settlementMode"]) {
  if (mode === "payment") return "Cab Upgrade Payment Required";
...
function getSettlementDescription(mode: CabQuote["settlementMode"]) {
  if (mode === "payment") {
    return "Selected cab has a higher fare. Continue to payment to confirm this change.";
  }

  if (mode === "wallet_credit") {
    return "Selected cab has a lower fare. Difference will be credited to Refund Wallet.";
  }

  return "Selected cab has the same fare as current booking.";
}