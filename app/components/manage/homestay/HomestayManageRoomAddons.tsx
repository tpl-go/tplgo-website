...
import { SectionTitle, formatPrice } from "../hotel/HotelManageShared";

export type HomestayRoomVariant = {
  id?: string;
  name?: string;
  price?: number;
  taxes?: number;
...
type Props = {
  ...
  nights: number;
  addOnsTotal: number;
  selectedVariant?: HomestayRoomVariant | null;
  variants?: HomestayRoomVariant[];
  activeVariantId?: string;
  onVariantChange: (variant: HomestayRoomVariant) => void;
  quote: HomestayRoomQuote;
  onContinue: () => void;
};

export default function HomestayManageRoomAddons({
  roomName,
  rooms,
  nights,
  addOnsTotal,
    ...
    <div className="space-y-5">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
          {variants.length === 0 ? (
            <div className="rounded-2xl border border-black/5 bg-[#f8f9fb] p-4 text-sm font-semibold text-[#6b7280]">
              Stay variants not available in payload.
            </div>
          ) : (
      ...
      <div className="rounded-[24px] border border-black/5 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 space-y-3">
          <Row label="Current Stay Total" value={formatPrice(quote.oldTotal)} />
          <Row label="New Stay Total" value={formatPrice(quote.newTotal)} />
          <Row
            label="Difference"
            value={
              quote.difference < 0
          ...
          {quote.settlementMode === "save" && (
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-[#6b7280]">
                  Same stay price. Changes can be saved directly.
                </p>
              </div>
...
function getSettlementTitle(mode: HomestayRoomQuote["settlementMode"]) {
...
function getSettlementDescription(mode: HomestayRoomQuote["settlementMode"]) {
  if (mode === "payment") {
    return "Selected stay is higher priced. Continue to payment to confirm this change.";
  }

  if (mode === "wallet_credit") {
    return "Selected stay is lower priced. Difference will be credited to Refund Wallet.";
  }

  return "Selected stay has the same price as current stay.";
}