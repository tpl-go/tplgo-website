...
type Props = {
  activeTab: CruiseManageTab;
  onTabChange: (tab: CruiseManageTab) => void;
  bookingId: string;
  cruiseTitle: string;
  routeLabel: string;
  sailingDateLabel: string;
  children: React.ReactNode;
};

const sidebarItems: SidebarItem[] = [
  { key: "summary", label: "Booking Summary", desc: "View cruise booking details" },
  { key: "traveller-details", label: "Traveller Details", desc: "Update traveller information" },
  { key: "contact-details", label: "Contact Details", desc: "Update email and phone" },
  { key: "special-request", label: "Special Request", desc: "Update cruise notes" },
  { key: "cabin-addons", label: "Cabin / Add-ons", desc: "Paid changes next", badge: "Paid" },
];

export default function CruiseManageLayout({
  activeTab,
  onTabChange,
  bookingId,
  cruiseTitle,
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f8f9fb]">
      <div className="bg-[#f8f9fb] px-3 pt-3 lg:hidden">
        <MobileInnerBack title="Cruise Manage" />
      </div>
            ...
            <div className="overflow-hidden rounded-[20px] border border-black/5 bg-white shadow-[0_10px_40px_rgba(0,0,0,0.04)] md:rounded-[28px]">
              ...
              <div className="p-3 lg:hidden">
                ...
                <select
                  value={activeTab}
                  onChange={(event) =>
                    onTabChange(event.target.value as CruiseManageTab)
                  }
              ...
              <div className="hidden border-t border-black/5 bg-[#fcfcfd] px-5 py-4 lg:block">
                <div className="rounded-2xl bg-[#f8f9fb] p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#4b5563]">
                    Cabin upgrades, paid add-ons, fare difference or refund wallet credit will be shown before final confirmation.
                  </p>
                </div>
              </div>
            </div>
