...
import HomestayRoomGuestSelector from "@/app/components/booking/homestay/search/HomestayRoomGuestSelector";
...
export default function HomestayBookingSearchBar({ homestay }: Props) {
  ...
  const defaultPropertyText = `${homestay.title}, ${homestay.city}, India`;
  ...
  useEffect(() => {
    ...
    const incomingChildren = Math.max(
    ...
    const safeRooms: Room[] = Array.from({ length: incomingRooms }, (_, index) => ({
      ...
      children: index === 0 ? incomingChildren : 0,
    }));
  ...
  const handleSearch = () => {
    ...
    if (normalizedInput === normalizedCurrent) {
      ...
      router.push(`/homestays/booking?${query.toString()}`);
    } else {
      const query = new URLSearchParams({
        city: city.trim(),
        checkIn: checkIn.toISOString(),
