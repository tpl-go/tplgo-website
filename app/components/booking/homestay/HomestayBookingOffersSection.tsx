...
type Props = {
  offers: HomestayOfferItem[];
  appliedOfferCode: string;
  onApplyOffer: (offer: HomestayOfferItem) => void;
  onRemoveOffer: () => void;
};

export default function HomestayBookingOffersSection({
  offers,
  appliedOfferCode,
  onApplyOffer,
  onRemoveOffer,
  ...
  const smartOffers = useMemo(() => {
    return SMART_OFFERS_DATA.filter((item) => {
      ...
      description:
        offer.description ||
        "Smart homestay offer available.",
  ...
  const mergedOffers = useMemo(() => {
    const map = new Map<string, HomestayOfferItem>();

    [...smartOffers, ...offers].forEach((offer) => {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#d9e2ec] bg-white shadow-[0_12px_34px_rgba(15,23,42,0.08)] md:rounded-[24px]">
      {/* HEADER */}
      <div className="border-b border-[#e5e7eb] bg-[linear-gradient(135deg,#fff7ed_0%,#ffffff_50%,#fff1e6_100%)] px-3 py-3 md:px-4 md:py-4">
