...
export default function CruiseOffersSection({
  ...
  const masterOffers = useMemo<CruiseOfferItem[]>(() => {
    return SMART_OFFERS_DATA.filter((item) => {
      if (!item.active) return false;

      return item.service === "cruise" || item.service === "all";
    }).map((offer) => ({
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-[24px] border border-[#d9e2ec] bg-white shadow-[0_12px_34px_rgba(15,23,42,0.08)]">
        ...
        <div className="grid gap-3">
          ...
          {masterOffers.length === 0 ? (
            <div className="rounded-[18px] border border-dashed border-[#e5e7eb] bg-[#fafafa] p-4 text-[13px] font-semibold text-[#6b7280]">
              No cruise offer available right now.
            </div>
          ) : null}
        </div>
      </div>
