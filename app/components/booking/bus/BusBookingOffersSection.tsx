...
type Props = {
  ...
  bookingValue: number;
  onApplyOffer: (offer: BusOfferItem) => void;
  onRemoveOffer: () => void;
};

function safeOfferAmount(amount: number, bookingValue: number) {
  return Math.min(
    Math.max(Math.round(Number(amount || 0)), 0),
    Math.max(Math.round(Number(bookingValue || 0)), 0)
  );
}

export default function BusBookingOffersSection({
  appliedOfferCode,
  bookingValue,
  onApplyOffer,
  onRemoveOffer,
  ...
  const masterOffers = useMemo<BusOfferItem[]>(() => {
    return SMART_OFFERS_DATA.filter((item) => {
      if (!item.active) return false;
      return item.service === "bus" || item.service === "all";
    }).map((offer) => {
      const discountAmount = safeOfferAmount(
        calculateSmartOfferDiscount(offer, bookingValue || 0),
        bookingValue || 0
      );
      ...
      return {
        ...
        title: offer.title,
        description:
          offer.description || offer.subtitle || "Smart bus offer available.",
        discountAmount,
      };
    });
  }, [bookingValue]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="mt-0.5 text-[12px] font-semibold text-[#6b7280]">
              AI matched bus savings on base fare
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid gap-3">
                ...
                <div className="relative flex items-start justify-between gap-3">
                  ...
                  <button
                    ...
                    onClick={() =>
                      onApplyOffer({
                        ...offer,
                        discountAmount: safeOfferAmount(
          ...
          {masterOffers.length === 0 ? (
            <div className="rounded-[18px] border border-dashed border-[#e5e7eb] bg-[#fafafa] p-4 text-[13px] font-semibold text-[#6b7280]">
              No bus offer available right now.
            </div>
          ) : null}
        </div>
      </div>
