...
import { BUS_DUMMY_RESULTS } from "@/app/lib/bus/busDummyData";
...
function getBaseBusFare(fromCity: string, toCity: string) {
  const matching = BUS_DUMMY_RESULTS.filter(
    (bus) =>
      normalize(bus.fromCity) === normalize(fromCity) &&
      normalize(bus.toCity) === normalize(toCity)
  );

  if (matching.length === 0) return 999;

  return Math.min(...matching.map((bus) => bus.price));
}

function getDynamicBusFareForDate(
  date: Date,
  fromCity: string,
  toCity: string
): number {
  ...
  const baseFare = getBaseBusFare(fromCity, toCity);

  const variations = [0, 70, 120, 0, 160, 40, 190, 0, 110, 55, 0, 145];
  const seasonalOffset =
    variations[Math.abs(dayIndex + routeSeed) % variations.length];
  const routeOffset = routeSeed % 180;

  return baseFare + seasonalOffset + routeOffset;
}
...
export default function BusDateFareStrip({
  ...
  const fareData: FareItem[] = useMemo(() => {
    return visibleDates.map((date) => ({
      date,
      price: getDynamicBusFareForDate(date, fromCity, toCity),
    }));
