...
export default function SmartOfferActivationModal({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
      <div className="relative w-full max-w-[560px] overflow-hidden rounded-[32px] bg-white shadow-2xl">
        <button
          onClick={onClose}
          className="absolute right-5 top-5 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow hover:bg-gray-100"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="bg-gradient-to-br from-orange-500 via-amber-500 to-yellow-400 p-6 text-white">
          ...
          <h2 className="mt-3 pr-10 text-2xl font-bold leading-snug">
            {offer.title}
          </h2>

          {offer.subtitle && (
            <p className="mt-2 text-sm text-white/90">{offer.subtitle}</p>
          )}
        </div>
        ...
        <div className="space-y-5 p-6">
          ...
          {offer.couponCode && (
            <div className="flex items-center justify-between rounded-2xl border border-dashed border-orange-300 bg-white px-4 py-3">
              <div>
                <div className="text-xs font-semibold text-gray-500">
                  Coupon Code
                </div>

                <div className="text-lg font-black tracking-widest text-orange-600">
                  {offer.couponCode}
                </div>
              </div>
