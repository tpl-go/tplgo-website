...
function ServicesStep({
  ...
  const [expandedDomain, setExpandedDomain] = useState<PartnerServiceDomainId | "all">(partnerServiceCatalog[0]?.id ?? "stay-accommodation");
  ...
  const visibleCatalog = useMemo(() => {
    const source = query.trim() ? filterPartnerServiceCatalog(query, eligibleCatalog) : eligibleCatalog;
    return expandedDomain === "all" || query.trim() ? source : source.filter((category) => category.id === expandedDomain);
  }, [eligibleCatalog, expandedDomain, query]);
  ...
  const selectedCodes = new Set(form.selectedServiceCodes);
  ...
  function toggleService(service: PartnerServiceCatalogueItem) {
    onChange((current) => {
      ...
      return {
        selectedServiceCodes: currentCodes.has(service.stableCode)
          ? current.selectedServiceCodes.filter((code) => code !== service.stableCode)
          : [...current.selectedServiceCodes, service.stableCode],
      };
    });
  }
      ...
      <div className="grid gap-5 p-5">
        ...
        <section className="rounded-xl border border-white/10 bg-[#11141a] p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <SectionHeading title="Find Services" detail="Search or open a domain to choose one or more services." />
            <label className="relative min-w-0 lg:w-[360px]">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl border border-white/10 bg-[#0f1217] pl-10 pr-10 text-sm font-semibold text-white outline-none placeholder:text-slate-500 focus:border-[#f97316] focus:ring-2 focus:ring-[#f97316]/25"
              />
              {query ? (
          ...
          <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            <button
              type="button"
              onClick={() => setExpandedDomain("all")}
              className={`rounded-xl border p-3 text-left ${expandedDomain === "all" ? "border-[#f97316]/50 bg-[#f97316]/10" : "border-white/10 bg-[#0f1217] hover:border-white/20"}`}
            >
              <p className="text-sm font-black text-white">All Services</p>
              <p className="mt-1 text-xs font-semibold text-slate-400">{selectedItems.length} selected</p>
            </button>
            {eligibleCatalog.map((category) => {
              ...
              return (
                <button
                  type="button"
                  key={category.id}
                  onClick={() => setExpandedDomain(category.id)}
                  className={`rounded-xl border p-3 text-left ${expandedDomain === category.id ? "border-[#f97316]/50 bg-[#f97316]/10" : "border-white/10 bg-[#0f1217] hover:border-white/20"}`}
                >
                  <p className="text-sm font-black text-white">{category.title}</p>
                  <p className="mt-1 text-xs font-semibold text-slate-400">{selectedCount ? `${selectedCount} selected` : `${category.services.length} services`}</p>
                </button>
              );
            })}
          </div>
        ...
        <section className="rounded-xl border border-white/10 bg-[#11141a] p-4">
          <SectionHeading title={query ? "Search Results" : "Service Options"} detail={query ? "Results include domain context so similar services are easy to compare." : "Select every service your business provides."} />
          <div className="mt-4 grid gap-3">
            {visibleCatalog.length ? visibleCatalog.map((category) => (
              <div key={category.id} className="rounded-xl border border-white/10 bg-[#0f1217] p-3">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-xs font-semibold text-slate-400">{category.description}</p>
                  </div>
                  <span className="text-xs font-black text-[#fed7aa]">{category.services.filter((service) => selectedCodes.has(service.id)).length} selected</span>
                </div>
                  ...
                  {category.services.map((service) => {
                    ...
                    return (
                      <button
                        ...
                        className={`min-h-[82px] rounded-xl border p-3 text-left transition ${selected ? "border-[#f97316]/70 bg-[#f97316]/12 shadow-[0_0_0_1px_rgba(249,115,22,0.22)]" : "border-white/10 bg-[#151922] hover:border-[#f97316]/35"}`}
                      >
              ...
              <div className="rounded-xl border border-white/10 bg-[#0f1217] p-5 text-sm font-semibold text-slate-300">
                No matching services. Try another search or request your service below.
              </div>
            )}
          </div>
        </section>
        ...
        <section className="rounded-xl border border-white/10 bg-[#11141a] p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <SectionHeading title="Your Selected Services" detail="These services will guide later verification checks and setup." />
            <button type="button" onClick={addRequest} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-[#f97316]/40 bg-[#f97316]/10 px-3 text-xs font-black text-[#fed7aa] hover:border-[#f97316]">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {selectedItems.length ? selectedItems.map((item) => (
              <span key={item.stableCode} data-selected-service={item.stableCode} className="inline-flex max-w-full items-center gap-2 rounded-full border border-[#f97316]/35 bg-[#f97316]/10 px-3 py-2 text-xs font-black text-[#fed7aa]">
                <span className="min-w-0">
                  <span className="block truncate">{item.name}</span>
                  <span className="block truncate text-[10px] font-bold text-slate-400">{domainTitleFor(item.domain)}</span>
                </span>
                {partnerServiceEligibleForApplication(item, countryCode, businessType) ? null : <span className="shrink-0 rounded-full bg-amber-400/15 px-2 py-0.5 text-[10px] text-amber-100">Needs attention</span>}
                <button type="button" onClick={() => toggleService(item)} aria-label={`Remove ${item.name}`} className="text-[#fed7aa]">
                  <X size={14} aria-hidden="true" />
                </button>
              </span>
            )) : <p className="text-sm font-semibold text-slate-400">No services selected yet.</p>}
          </div>
