...
export type InsuranceBackendCheckoutRefs = {
  backendCheckoutId?: string;
  backendBookingId?: string;
  backendPaymentId?: string;
  backendRequestId?: string;
  backendServiceType?: "insurance";
  backendCheckoutStatus?: string;
};

export type InsuranceBackendAttemptResult<TPayload> = {
  payload: TPayload;
  refs: InsuranceBackendCheckoutRefs;
  attempted: boolean;
  ok: boolean;
};

const INSURANCE_START_KEY_PREFIX = "tpl_insurance_backend_start_key_";
const INSURANCE_CONFIRM_KEY_PREFIX = "tpl_insurance_backend_confirm_key_";

export function isInsuranceBackendCheckoutReady(): boolean {
  return isBackendCheckoutEnabled("insurance") && isTplApiConfigured();
}

export async function startInsuranceBackendCheckout<TPayload extends Record<string, unknown>>(
  rawPayload: TPayload
): Promise<InsuranceBackendAttemptResult<TPayload>> {
  if (!isInsuranceBackendCheckoutReady()) {
    return { payload: rawPayload, refs: {}, attempted: false, ok: false };
  }

  const hash = hashPayload(stripVolatileBackendFields(rawPayload));
  const guarded = await prepareBackendCheckoutWalletPayload("insurance", rawPayload);
  const guardedPayload = guarded.payload as TPayload;
  const idempotencyKey = readOrCreateSessionKey(
    `${INSURANCE_START_KEY_PREFIX}${hash}`,
    `insurance:start:${hash}`
  );

  const result = await startServiceCheckout("insurance", guardedPayload, idempotencyKey);

  if (!result.ok) {
    logInsuranceBackendDebug("Insurance backend checkout start failed", {
    ...
    throw new Error("Insurance backend checkout start failed.");
  }

  const refs: InsuranceBackendCheckoutRefs = {
  ...
  logInsuranceBackendDebug("Insurance backend checkout start succeeded", refs);
...
export async function confirmInsuranceBackendCheckout<TPayload extends Record<string, unknown>>(
  ...
  const sessionKey = `${INSURANCE_CONFIRM_KEY_PREFIX}${checkoutId}_${paymentAttemptId}`;
  const idempotencyKey = readOrCreateSessionKey(
    sessionKey,
    `insurance:confirm:${checkoutId}:${paymentAttemptId}`
  );
  ...
  const result = await confirmServiceCheckout(
    ...
    checkoutId,
    {
      ...
      metadata: {
        source: "insurance_frontend_phase_3_6",
  ...
  if (!result.ok) {
    logInsuranceBackendDebug("Insurance backend checkout confirm failed", {
      requestId: result.requestId,
      error: result.error,
    });
    ...
    throw new Error("Insurance backend checkout confirmation failed.");
  }
  ...
  logInsuranceBackendDebug("Insurance backend checkout confirm succeeded", refs);
...
function removeEmptyRefs(refs: InsuranceBackendCheckoutRefs): InsuranceBackendCheckoutRefs {
  return Object.fromEntries(
    Object.entries(refs).filter(([, value]) => typeof value !== "undefined" && value !== "")
  ) as InsuranceBackendCheckoutRefs;
}

function logInsuranceBackendDebug(message: string, value: unknown): void {
