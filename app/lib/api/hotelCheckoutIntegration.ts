...
export type HotelBackendCheckoutRefs = {
  backendCheckoutId?: string;
  backendBookingId?: string;
  backendPaymentId?: string;
  backendRequestId?: string;
  backendServiceType?: "hotel";
  backendCheckoutStatus?: string;
};

export type HotelBackendAttemptResult<TPayload> = {
  payload: TPayload;
  refs: HotelBackendCheckoutRefs;
  attempted: boolean;
  ok: boolean;
};

const HOTEL_START_KEY_PREFIX = "tpl_hotel_backend_start_key_";
const HOTEL_CONFIRM_KEY_PREFIX = "tpl_hotel_backend_confirm_key_";

export function isHotelBackendCheckoutReady(): boolean {
  return isBackendCheckoutEnabled("hotel") && isTplApiConfigured();
}

export async function startHotelBackendCheckout<
  TPayload extends Record<string, unknown>,
>(rawPayload: TPayload): Promise<HotelBackendAttemptResult<TPayload>> {
  if (!isHotelBackendCheckoutReady()) {
    return { payload: rawPayload, refs: {}, attempted: false, ok: false };
  }

  const hash = hashPayload(stripVolatileBackendFields(rawPayload));
  const guarded = await prepareBackendCheckoutWalletPayload("hotel", rawPayload);
  const guardedPayload = guarded.payload as TPayload;
  const idempotencyKey = readOrCreateSessionKey(
    `${HOTEL_START_KEY_PREFIX}${hash}`,
    `hotel:start:${hash}`
  );

  const result = await startServiceCheckout("hotel", guardedPayload, idempotencyKey);

  if (!result.ok) {
    logHotelBackendDebug("Hotel backend checkout start failed", {
    ...
    throw new Error("Hotel backend checkout start failed.");
  }

  const refs: HotelBackendCheckoutRefs = {
  ...
  logHotelBackendDebug("Hotel backend checkout start succeeded", refs);
...
export async function confirmHotelBackendCheckout<
  TPayload extends Record<string, unknown>,
>(
  confirmationPayload: TPayload
): Promise<HotelBackendAttemptResult<TPayload>> {
  if (!isHotelBackendCheckoutReady()) {
  ...
  const sessionKey = `${HOTEL_CONFIRM_KEY_PREFIX}${checkoutId}_${paymentAttemptId}`;
  const idempotencyKey = readOrCreateSessionKey(
    sessionKey,
    `hotel:confirm:${checkoutId}:${paymentAttemptId}`
  );
  ...
  const result = await confirmServiceCheckout(
    ...
    checkoutId,
    {
      ...
      metadata: {
        source: "hotel_frontend_phase_3_8",
  ...
  if (!result.ok) {
    logHotelBackendDebug("Hotel backend checkout confirm failed", {
      requestId: result.requestId,
      error: result.error,
    });
    ...
    throw new Error("Hotel backend checkout confirmation failed.");
  }
  ...
  logHotelBackendDebug("Hotel backend checkout confirm succeeded", refs);
...
function removeEmptyRefs(refs: HotelBackendCheckoutRefs): HotelBackendCheckoutRefs {
  return Object.fromEntries(
    Object.entries(refs).filter(
      ([, value]) => typeof value !== "undefined" && value !== ""
    )
  ) as HotelBackendCheckoutRefs;
}

function logHotelBackendDebug(message: string, value: unknown): void {
