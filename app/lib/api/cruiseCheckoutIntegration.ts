...
export type CruiseBackendCheckoutRefs = {
...
export type CruiseBackendAttemptResult<TPayload> = {
  payload: TPayload;
  refs: CruiseBackendCheckoutRefs;
  attempted: boolean;
  ok: boolean;
};

const CRUISE_START_KEY_PREFIX = "tpl_cruise_backend_start_key_";
const CRUISE_CONFIRM_KEY_PREFIX = "tpl_cruise_backend_confirm_key_";

export function isCruiseBackendCheckoutReady(): boolean {
  return isBackendCheckoutEnabled("cruise") && isTplApiConfigured();
}

export async function startCruiseBackendCheckout<
  TPayload extends Record<string, unknown>,
>(rawPayload: TPayload): Promise<CruiseBackendAttemptResult<TPayload>> {
  if (!isCruiseBackendCheckoutReady()) {
  ...
  if (!result.ok) {
    logCruiseBackendDebug("Cruise backend checkout start failed", {
    ...
    throw new Error("Cruise backend checkout start failed.");
  }

  const refs: CruiseBackendCheckoutRefs = {
  ...
  logCruiseBackendDebug("Cruise backend checkout start succeeded", refs);
...
export async function confirmCruiseBackendCheckout<
  TPayload extends Record<string, unknown>,
>(
  confirmationPayload: TPayload
): Promise<CruiseBackendAttemptResult<TPayload>> {
  if (!isCruiseBackendCheckoutReady()) {
  ...
  const sessionKey = `${CRUISE_CONFIRM_KEY_PREFIX}${checkoutId}_${paymentAttemptId}`;
  const idempotencyKey = readOrCreateSessionKey(
    sessionKey,
    `cruise:confirm:${checkoutId}:${paymentAttemptId}`
  );
  ...
  if (!result.ok) {
    logCruiseBackendDebug("Cruise backend checkout confirm failed", {
      requestId: result.requestId,
      error: result.error,
    });
    ...
    throw new Error("Cruise backend checkout confirmation failed.");
  }
  ...
  logCruiseBackendDebug("Cruise backend checkout confirm succeeded", refs);
...
function removeEmptyRefs(
  refs: CruiseBackendCheckoutRefs
): CruiseBackendCheckoutRefs {
  return Object.fromEntries(
    Object.entries(refs).filter(
      ([, value]) => typeof value !== "undefined" && value !== ""
    )
  ) as CruiseBackendCheckoutRefs;
}

function logCruiseBackendDebug(message: string, value: unknown): void {
