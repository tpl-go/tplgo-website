...
test("validates every server outcome without accepting arbitrary destinations", () => {
  for (const outcome of ["APPLICATION_STATUS", "CORRECTIONS", "SETUP_PENDING", "ACTIVE", "RESTRICTED", "SELECTION_REQUIRED", "NO_LINKED_PROFILE"] as const) {
    ...
    expect(partnerAccessDestination(value)).toBe(outcome === "CORRECTIONS" ? "/partner-preview?step=review_submit&organizationId=test-org" : null);
  }
...
test("submitted, active, setup and restricted outcomes stay in the Partner access shell", () => {
  for (const outcome of ["APPLICATION_STATUS", "SETUP_PENDING", "ACTIVE", "RESTRICTED"] as const) {
    expect(partnerAccessDestination(parsePartnerAccess({ outcome, organizationId: "test-org", step: "review_submit" }))).toBeNull();
  }
});
