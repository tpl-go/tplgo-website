...
describe('HOTEL-M3B request contract',()=>{
 it('accepts only the bounded versioned request shape',()=>{const value={contractVersion:1,audience:'partner',organizationId:'o',canSubmit:true,canReview:true,paymentBoundary:'No finance.',requests:[{id:'r',requestRef:'TPL-M3B',version:1,timeline:[]}],freshness:{asOf:'2026-09-24T00:00:00Z',timezone:'UTC'}};expect(validHotelBookingRequestSnapshot(value)).toBe(true);expect(validHotelBookingRequestSnapshot({...value,contractVersion:2})).toBe(false);});
