...
describe('HOTEL-M3B approved modification presentation',()=>{
 ...
 it('keeps the Partner modified marker after revised acknowledgement',()=>{const workspace=readFileSync(resolve(process.cwd(),'app/components/partner/PartnerHotelBookingWorkspace.tsx'),'utf8');expect(workspace).toContain("event.eventType==='hotel.booking.modification.applied'");expect(workspace.match(/Modified booking/g)?.length).toBe(2);});
});
